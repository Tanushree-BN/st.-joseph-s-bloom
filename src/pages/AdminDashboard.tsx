...
import { Image as ImageIcon, Newspaper, Mail, GraduationCap, Plus, Trash2, Edit2, Eye, LogOut, Check, X } from "lucide-react";
import {
        ...
        <div className="container-school">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { icon: GraduationCap, label: "Admissions", value: `${pendingAdmissions} pending`, color: "text-secondary" },
            ].map((s, i) => (
          ...
          <Tabs defaultValue="gallery">
                  ...
                  <div key={item.id} className="bg-card rounded-lg p-4 flex items-center justify-between gap-4 card-hover">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${item.isUpcoming ? "bg-secondary/20 text-secondary" : "bg-accent text-accent-foreground"}`}>
                          {item.isUpcoming ? "Upcoming" : item.type}
                        </span>
            ...
            <TabsContent value="admissions">
              <h2 className="font-display text-xl font-semibold mb-4">Admission Applications ({admissions.length})</h2>
              {admissions.length === 0 ? (
                <p className="text-muted-foreground font-body text-center py-10">No applications yet.</p>
              ) : (
                <div className="space-y-3">
                  {admissions.map(a => (
                    <div key={a.id} className="bg-card rounded-lg p-4 flex items-center justify-between gap-4 card-hover">
                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setViewAdmission(a)}>
                        <div className="flex items-center gap-2 mb-1">
                          <p className="font-display font-semibold text-sm">{a.studentName}</p>
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                            a.status === "pending" ? "bg-secondary/20 text-secondary" :
                            a.status === "accepted" ? "bg-green-100 text-green-700" :
                            a.status === "rejected" ? "bg-destructive/10 text-destructive" :
                            "bg-accent text-accent-foreground"
                          }`}>{a.status}</span>
                        </div>
                        <p className="text-xs text-muted-foreground">Grade: {a.gradeApplying} | Parent: {a.parentName}</p>
                        <p className="text-[10px] text-muted-foreground mt-1">{a.date}</p>
                      </div>
                      <div className="flex gap-1 shrink-0">
                        {a.status === "pending" && (
                          <>
                            <button onClick={() => { updateAdmissionStatus(a.id, "accepted"); refreshData(); toast.success("Accepted!"); }} className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center hover:bg-green-200"><Check className="w-3 h-3 text-green-700" /></button>
                            <button onClick={() => { updateAdmissionStatus(a.id, "rejected"); refreshData(); toast.success("Rejected."); }} className="w-8 h-8 rounded-full bg-destructive/10 flex items-center justify-center hover:bg-destructive/20"><X className="w-3 h-3 text-destructive" /></button>
                          </>
                        )}
      ...
      <Dialog open={newsDialog} onOpenChange={setNewsDialog}>
        <DialogContent>
          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto">
            ...
            <Input placeholder="Date" value={nForm.date} onChange={(e) => setNForm({ ...nForm, date: e.target.value })} />
            <Textarea placeholder="Description" value={nForm.description} onChange={(e) => setNForm({ ...nForm, description: e.target.value })} />
            <Input placeholder="Image URL" value={nForm.image} onChange={(e) => setNForm({ ...nForm, image: e.target.value })} />
            ...
            {nForm.isUpcoming && <Input placeholder="Time" value={nForm.time} onChange={(e) => setNForm({ ...nForm, time: e.target.value })} />}
        ...
        <Dialog open={!!editNews} onOpenChange={() => setEditNews(null)}>
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <Input value={editNews.date} onChange={(e) => setEditNews({ ...editNews, date: e.target.value })} />
        ...
        <Dialog open={!!viewAdmission} onOpenChange={() => setViewAdmission(null)}>
          <DialogContent>
            ...
            <div className="space-y-2 text-sm">
              <p><strong>Parent:</strong> {viewAdmission.parentName}</p>
              ...
              <p><strong>Address:</strong> {viewAdmission.address || "N/A"}</p>
              <p><strong>Previous School:</strong> {viewAdmission.previousSchool || "N/A"}</p>
              <p><strong>Status:</strong> <span className="font-semibold">{viewAdmission.status}</span></p>
              <p><strong>Submitted:</strong> {viewAdmission.date}</p>
            </div>
          </DialogContent>
        </Dialog>
      )}
